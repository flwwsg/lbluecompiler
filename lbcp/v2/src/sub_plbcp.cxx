 ...
 * revision:		Sat Apr 18 20:16:02 CST 2015, originally written by DJW
						Mon May 11 11:21:09 CST 2015, 
						replacing rs with rt, for lw instruction.
...
// 	sptbl.push_back(*(new spins("01", pins::mov)));
// 	sptbl.push_back(*(new spins("90", pins::subi)));
}

//find key word for step a
...
void replacing_ins(vector<rdst>& regtbl, vector<pins>& ppins)
{
	for(int i = 0; i < ppins.size(); ++i){
		ppins[i].set_field(pins::rd, find_rdst(ppins[i][pins::rd], regtbl));
		ppins[i].set_field(pins::rs, find_rdst(ppins[i][pins::rs], regtbl));
		ppins[i].set_field(pins::rt, find_rdst(ppins[i][pins::rt], regtbl));
	}
}
